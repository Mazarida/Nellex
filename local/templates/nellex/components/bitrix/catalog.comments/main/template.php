...
<?if (!$templateData['BLOG']['BLOG_FROM_AJAX']):?>
	...
	if ($arResult['BLOG_USE']) {
		...
		$arData["BLOG"] =  array(
			"NAME" => ($arParams['BLOG_TITLE'] != '' ? $arParams['BLOG_TITLE'] : Loc::getMessage('IBLOCK_CSC_TAB_COMMENTS')),
			"ACTIVE" => "Y",
			"CONTENT" => '<div id="bx-cat-soc-comments-blg_'.$arResult['ELEMENT']['ID'].'">'.Loc::getMessage("IBLOCK_CSC_COMMENTS_LOADING").'</div>'
		);
	}
	...
	if ($arParams["VK_USE"] == "Y") {
		$arData["VK"] = array(
			"NAME" => isset($arParams["VK_TITLE"]) && trim($arParams["VK_TITLE"]) != "" ? $arParams["VK_TITLE"] : Loc::getMessage("IBLOCK_CSC_TAB_VK"),
			"CONTENT" => '
					...
					BX.load([\'https://vk.com/js/api/openapi.js?142\'], function(){
						if (!!window.VK) {
							...
							VK.Widgets.Comments("vk_comments",
								{
									pageUrl: "'.$arResult["URL_TO_COMMENT"].'",'.(isset($arParams["COMMENTS_COUNT"]) ? "limit: ".$arParams["COMMENTS_COUNT"]."," : "").
										(isset($arResult["WIDTH"]) ? "width: ".($arResult["WIDTH"] - 20)."," : ""). 'attach: false,
	...
	<?if (!empty($arData)):?>
		...
		<div id="<?=$templateData['TABS_FRAME_ID']; ?>" class="bx_soc_comments_div bx_important <?=$templateData['TEMPLATE_CLASS']; ?>">
			...
			$tabIDList = array();
			?>
			<div id="<?=$templateData['TABS_ID']; ?>" class="bx-catalog-tab-section-container"<?=isset($arResult["WIDTH"]) ? ' style="width: '.$arResult["WIDTH"].'px;"' : ''?>>
