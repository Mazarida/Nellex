...
$status_item = '';
if (isset($item['DISPLAY_PROPERTIES']['NEW'])) {
	$status_item .= ' new';
}
...
<div class="product-item cls-tst_____cata__prod<?=$status_item?>">
	<?/*<span class="product-item-image-slider-slide-container slide" id="<?=$itemIds['PICT_SLIDER']?>"
		<?=($showSlider ? '' : 'style="display: none;"')?> data-slider-interval="<?=$arParams['SLIDER_INTERVAL']?>" data-slider-wrap="true">
		<?if ($showSlider):?>
	...
	<a href="<?=$item['DETAIL_PAGE_URL']?>" class="cata__image" id="<?=$itemIds['PICT']?>" style="background-image: url('<?=$item['PREVIEW_PICTURE']['SRC']?>');"></a>
	...
	<div class="cata__row-name">
		<a href="<?=$item['DETAIL_PAGE_URL']?>" class="cata__name"><?=$item['DISPLAY_PROPERTIES']['BRAND']['DISPLAY_VALUE']?> | <?=$item['NAME']?></a>

		<div class="flex-row cata__add-cart-like">
			<?if (stripos($APPLICATION->GetCurDir(), '/catalog/favorite/') === false):?>
				<a href="javascript:void(0);" class="cata__like" data-product_id="<?=$item['ID']?>"></a>
			<?else:?>
				<a href="javascript:void(0);" class="cata__like-delete" data-product_id="<?=$item['ID']?>"></a>
			<?endif;?>

			<?if ($actualItem['CAN_BUY']):?>
				<?/*<a class="cata__add-cart" id="<?=$itemIds['BUY_LINK']?>" href="javascript:void(0)" rel="nofollow"></a>*/?>
				<a href="<?=$item['DETAIL_PAGE_URL']?>" class="cata__add-cart"></a>
			<?endif;?>
		</div>

		<div class="cata__price">
			<?=$price['PRINT_RATIO_PRICE'];?>
			<?if ($price['RATIO_PRICE'] < $price['RATIO_BASE_PRICE']):?>
				<div class="cata__prix-old"><?=$price['PRINT_RATIO_BASE_PRICE']?></div>
			<?endif;?>
			<?if ($price['PERCENT'] > 0):?>
		...
		<?if (isset($item['DISPLAY_PROPERTIES']['SIZE'])):?>
			<div class="cata__as-size"><?=Loc::getMessage('AVAILABLE_SIZES_TEXT')?>: <?=is_array($item['DISPLAY_PROPERTIES']['SIZE']['DISPLAY_VALUE']) ?
					implode(', ', $item['DISPLAY_PROPERTIES']['SIZE']['DISPLAY_VALUE']) : $item['DISPLAY_PROPERTIES']['SIZE']['DISPLAY_VALUE']?></div>
		<?endif;?>

		<?if (floatval($item["PROPERTIES"]["rating"]["VALUE"])):?>
			<div class="rating__stars stars<?=round($item["PROPERTIES"]["rating"]["VALUE"])?>"></div>
		<?else:?>
