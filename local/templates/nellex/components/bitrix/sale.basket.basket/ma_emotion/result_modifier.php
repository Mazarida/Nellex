...
if (is_array($arResult["GRID"]["ROWS"])) {
	...
	foreach ($arResult['GRID']['ROWS'] as $key => $item) {
		...
		if (\Bitrix\Main\Loader::includeModule('iblock')) {
			...
			$arFilter = array("IBLOCK_ID" => IBLOCK_ID__CATALOG, "ID" => $item['PRODUCT_ID']);
			$res = CIBlockElement::GetList(array(), $arFilter, false, false, $arSelect);
			while ($ob = $res->GetNextElement()) {
				$arColor[] = $ob->fields["PROPERTY_COLOR_VALUE"];
			}
		}

//		PR($arColor);
		if (\Bitrix\Main\Loader::includeModule("highloadblock") && count($arColor)) {
			if ($entity_data_class = GetEntityDataClass(HLBLOCK_ID__COLORS)) {
				...
				while ($color = $rsData->fetch()) {
					$arColorSrc[$color['UF_XML_ID']] = CFile::GetPath($color['UF_FILE']);
				}
			}

			if (count($arColorSrc)) {
				foreach ($arColor as $color) {
					$arResult['GRID']['ROWS'][$key]['PROPERTY_COLOR_SRC'][] = $arColorSrc[$color];
				}
			}
		}
	}
