...
if($_REQUEST["ALTASIB_AJAX_CALL"]=="Y" && $arParams["SHOW_ALL"])
    $APPLICATION->RestartBuffer();

if($_POST["ALTASIB_AJAX_CALL"]=="Y")
{
	$APPLICATION->RestartBuffer();
	$RID = (int)$_POST["RID"];
...
if($_SERVER["REQUEST_METHOD"] == "POST" && check_bitrix_sessid() && $_POST["RID"]>0 && $_POST["ACTION"]=="COMPLAINT_ADD")
{
	...
	{
		$strWarning = GetMessage("ALTASIB_REVIEW_CP_LIST_ABUSE_ERROR_AUTH");
        require_once('complaint.php');
        die();
	}
	...
	if(!$USER->IsAuthorized())
	{
			...
			if(strlen($SENDER_NAME)==0)
					$strWarning .= GetMessage("ALTASIB_REVIEW_CP_LIST_ABUSE_ERROR_SENDER_NAME")."<br />";

			$SENDER_EMAIL = htmlspecialchars($_POST["SENDER_EMAIL"]);
			if(!check_email($SENDER_EMAIL))
			{
					$strWarning .= GetMessage("ALTASIB_REVIEW_CP_LIST_ABUSE_ERROR_SENDER_EMAIL")."<br />";
			}
	}    
	$MESSAGE = htmlspecialchars($_POST["comment"]);
	if(strlen($MESSAGE)==0)
		$strWarning .= GetMessage("ALTASIB_REVIEW_CP_LIST_ABUSE_ERROR_MESSAGE")."<br />";
...
?>
        ...
        alert('<?=GetMessage("ALTASIB_REVIEW_T_REVIEW_LIST_REPORT_OK")?>');
...
$cache_path = "/".SITE_ID."/altasib/review.list/".$arParams["ELEMENT_ID"]."/";
...
else
{
    if ($arParams["CACHE_TIME"] > 0)
		...
		if($arElement = $rsElement->GetNext())
		{
            ...
            $arResult["ALL_CNT"] = aReview::Count(array_merge($arRFilter,array('APPROVED'=>'Y')));
            $limit = !$arParams["SHOW_ALL"] ? $arParams["REVIEWS_ON_PAGE"] : null;
            $obReview = aReview::GetList(Array("ID"=>"DESC"),$arRFilter,array('*'),array(),$limit);
			while($arReview = $obReview->fetch())
			{
                $arReview['POST_DATE'] = $arReview['POST_DATE']->toString();
                unset($arReview['ALTASIB_REVIEW_REVIEW_USER_PERSONAL_BIRTHDAY']);
                unset($arReview['ALTASIB_REVIEW_REVIEW_USER_LAST_LOGIN_SHORT']);
                unset($arReview['ALTASIB_REVIEW_REVIEW_USER_LAST_LOGIN']);
                unset($arReview['ALTASIB_REVIEW_REVIEW_USER_DATE_REG_SHORT']);
                unset($arReview['ALTASIB_REVIEW_REVIEW_USER_DATE_REGISTER']);
                unset($arReview['ALTASIB_REVIEW_REVIEW_USER_LAST_ACTIVITY_DATE']);
                ...
                if($arReview["USER_ID"]>0)
                {
                   ...
                   {
                        case "LOGIN":
                            $arReview["AUTHOR_NAME"] = $arReview["ALTASIB_REVIEW_REVIEW_USER_LOGIN"]; 
                        break;
                    
                        case "NAME":
                            $arReview["AUTHOR_NAME"] = $arReview["ALTASIB_REVIEW_REVIEW_USER_NAME"]; 
                        break;
                        case "NAME_LAST_NAME":
                            $arReview["AUTHOR_NAME"] = $arReview["ALTASIB_REVIEW_REVIEW_USER_NAME"]." ".$arReview["ALTASIB_REVIEW_REVIEW_USER_LAST_NAME"];                                
                        break;
                        case "LOGIN_NAME":
                            $arReview["AUTHOR_NAME"] = "(".$arReview["ALTASIB_REVIEW_REVIEW_USER_LOGIN"].") ".$arReview["ALTASIB_REVIEW_REVIEW_USER_NAME"];
                        break;                                                                                    
                        case "LOGIN_NAME_LAST_NAME":
                            $arReview["AUTHOR_NAME"] = "(".$arReview["ALTASIB_REVIEW_REVIEW_USER_LOGIN"].") ".$arReview["ALTASIB_REVIEW_REVIEW_USER_NAME"]." ".$arReview["ALTASIB_REVIEW_REVIEW_USER_LAST_NAME"];
                        break;                                                                
                   } 
                }
                
                $ts = MakeTimeStamp($arReview["DATE"], CSite::GetDateFormat());			 
				$arReview["POST_DATE_FORMAT"] = aReview::FormatDate($arParams["POST_DATE_FORMAT"],$ts);
                $arReview["FILES"] = aReviewFile::GetFiles($arReview["ID"]);
                ...
                $arReview["USER_FIELDS"] = $GLOBALS["USER_FIELD_MANAGER"]->GetUserFields("ALTASIB_REVIEW",$arReview["ID"],LANGUAGE_ID);
...
if(isset($this->__template->__folder) && strlen($this->__template->__folder)>0)
				...
				$path = "/bitrix/components/altasib/review.list/templates/".$this->__templateName;
$APPLICATION->AddHeadString("
<script>
var ReviewModMessages = {
	'REVIEW_LIST_MODER_APP': '".GetMessage("ALTASIB_REVIEW_T_REVIEW_LIST_MODER_APP")."',
	'REVIEW_LIST_MODER_HIDE': '".GetMessage("ALTASIB_REVIEW_T_REVIEW_LIST_MODER_HIDE")."',
