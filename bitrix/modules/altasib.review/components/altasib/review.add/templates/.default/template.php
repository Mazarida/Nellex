...
?>
<div>
<?if($isHidden):?><div class="alx_add_reviews_a" id="review_show_form"><a href="javascript:void(0)" onclick="ShowReviewForm();"><?echo strlen($arParams["ADD_TITLE"]) ==0 ? GetMessage("ALTASIB_TP_ADD") : $arParams["ADD_TITLE"]?></a></div><?endif;?>
	...
	<div class="alx_reviews_form">
        <?if(!$USER->IsAuthorized()):?>
    		<div class="alx_reviews_form_item_pole alx_reviews_form_poles_small">
    			<div class="alx_reviews_form_pole_name"><span class="requred_txt">*</span> <?=GetMessage("ALTASIB_TP_NAME")?>:</div>
    			<div class="alx_reviews_form_inputtext_bg"><input type="text" name="AUTHOR_NAME" value="<?=$arResult["arMessage"]["AUTHOR_NAME"];?>" /></div>
				<div class="alx_clear_block">&nbsp;</div>
    		</div>
    		<div class="alx_reviews_form_item_pole alx_reviews_form_poles_small">
    			<div class="alx_reviews_form_pole_name"><span class="requred_txt">*</span> <?=GetMessage("ALTASIB_TP_EMAIL")?>:</div>
        ...
        <div class="alx_reviews_form_item_pole_textarea">
			...
			<div class="alx_reviews_form_textarea_bg">
                <?=aReview::ShowLHE("MESSAGE_PLUS",$arResult["arMessage"]["MESSAGE_PLUS"],"MESSAGE_PLUS");?>
			</div>
		...
		<div class="alx_reviews_form_item_pole_textarea">
			...
			<div class="alx_reviews_form_textarea_bg">
                <?=aReview::ShowLHE("MESSAGE_MINUS",$arResult["arMessage"]["MESSAGE_MINUS"],"MESSAGE_MINUS");?>
			</div>
            <?/*if($arParams["SHOW_CNT"]):?>
            <div class="alx_reviews_form_item_pole_textarea_dop_txt">
        ...
        <?endif;?>
		<div class="alx_reviews_form_item_pole_textarea">
			<div class="alx_reviews_form_pole_name"><span class="requred_txt">*</span> <?=GetMessage("ALTASIB_TP_COMMENT")?>:</div>
			<div class="alx_reviews_form_textarea_bg">
                <?=aReview::ShowLHE("MESSAGE",$arResult["arMessage"]["MESSAGE"],"MESSAGE",200,"oLHErwc");?>
			</div>
			<div class="alx_reviews_form_item_pole_textarea_dop_txt">
				<?if($arParams["MIN_LENGTH"]>0):?><?=GetMessage("ALTASIB_TP_MIN_L")?> <span class="alx_reviews_red_txt"><?=$arParams["MIN_LENGTH"]?></span> <?=GetMessage("ALTASIB_TP_S")?>. <br /><?endif;?>
				<?if($arParams["MAX_LENGTH"]>0):?><?=GetMessage("ALTASIB_TP_MAX_L")?> <?=$arParams["MAX_LENGTH"]?> <?=GetMessage("ALTASIB_TP_S")?>. <?=GetMessage("ALTASIB_TP_R")?>: <span id="review_max_cnt" class="alx_reviews_red_txt"><?=$arParams["MAX_LENGTH"]?></span>.<?endif;?>
                <?if($arParams["SHOW_CNT"]):?><?=GetMessage("ALTASIB_TP_SCORE")?><span id="review_cnt_c" class="alx_reviews_red_txt">0</span><?endif;?>
			</div>
            ...
            <div><?=GetMessage('ALTASIB_TP_FILE_EXT',Array('#FILE_EXT#'=>$arParams["UPLOAD_FILE_TYPE"]))?></div>                                    
            <div><?=GetMessage('ALTASIB_TP_FILE_SIZE',Array('#FILE_SIZE#'=>CFile::FormatSize(intval($arParams["UPLOAD_FILE_SIZE"]*1024), 0)))?></div>
            <?endif;?>
            
			<div class="alx_clear_block">&nbsp;</div>
		</div>
        <div class="alx_reviews_form_vote" id="alx_reviews_form_vote">
			<div class="alx_reviews_form_vote_group_name"><span class="requred_txt">*</span> <?=GetMessage("ALTASIB_TP_VOTE")?>:</div>
        ...
        <?if ($arParams["USE_CAPTCHA"] == "Y"):?>
            <div class="alx_reviews_form_captcha">
    				<div class="alx_reviews_form_pole_name"><span class="requred_txt">*</span> <?=GetMessage('ALTASIB_TP_CONFIRM')?><br />&nbsp; <?=GetMessage('ALTASIB_TP_CONFIRM_CODE')?>:</div>
		...
		<div class="alx_reviews_subscribe">
            ...
            <?/*</div>*/?><?=GetMessage("ALTASIB_TP_SUBSCRIBE")?><?/*</div>*/?></div>
		</div>
        <?endif;?>
		<div class="alx_reviews_form_requred_block">
				<span class="requred_txt">*</span> <?=GetMessage("ALTASIB_TP_R_FIELDS")?>
			</div>
	</div>
	<div class="alx_reviews_block_border"></div>
	<div class="alx_reviews_form_submit_block">
			<div class="alx_reviews_form_item_submit"><input type="submit" name="review_add_btn" value="<?=GetMessage("ALTASIB_TP_SEND")?>" /></div>		
	</div>    
	</div>
</form>
</div>