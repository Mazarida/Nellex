...
if($arParams["ONLY_AUTH_SEND"] && !$USER->IsAuthorized())
{
	...
	echo GetMessage("ALTASIB_REVIEW_ADD_CP_ONLY_AUTH_SEND",array("#REG_URL#"=>$arParams['REG_URL'].(strstr($arParams['REG_URL'],"?") ? "&" : "?")."backurl=".$back_url));
	return;
}
...
if($_SERVER["REQUEST_METHOD"] == "POST" && check_bitrix_sessid() && (strlen($_POST["review_add_btn"])>0 || $_POST["ACTION"]=="EDIT"))
{
        ...
        if($arParams["ALLOW_TITLE"] && !$arParams["COMMENTS_MODE"])
        {
			...
			if(strlen($TITLE)==0 || strlen($TITLE)>255)
				$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_TITLE")."<br>";                
        }    
        $MESSAGE_PLUS_TEXT;
        $MESSAGE_MINUS_TEXT;
        if(!$arParams["COMMENTS_MODE"])
        {
			...
			elseif($arParams["PLUS_TEXT_MIN_LENGTH"]>0 && strlen($MESSAGE_PLUS_TEXT)<$arParams["PLUS_TEXT_MIN_LENGTH"])
				$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_PLUS_TEXT_MIN_LENGTH",Array("#CNT#"=>$arParams["PLUS_TEXT_MIN_LENGTH"]))."<br>";				
			elseif($arParams["PLUS_TEXT_MAX_LENGTH"]>0 && strlen($MESSAGE_PLUS_TEXT)>$arParams["PLUS_TEXT_MAX_LENGTH"])
				$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_PLUS_TEXT_MAX_LENGTH",Array("#CNT#"=>$arParams["PLUS_TEXT_MAX_LENGTH"]))."<br>";
            
			if($arParams["MINUS_TEXT_MIN_LENGTH"]>0 && strlen($MESSAGE_MINUS_TEXT)<$arParams["MINUS_TEXT_MIN_LENGTH"])
				$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_MINUS_TEXT_MIN_LENGTH",Array("#CNT#"=>$arParams["MINUS_TEXT_MIN_LENGTH"]))."<br>";				
			elseif($arParams["MINUS_TEXT_MAX_LENGTH"]>0 && strlen($MESSAGE_MINUS_TEXT)>$arParams["MINUS_TEXT_MAX_LENGTH"])
				$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_MINUS_TEXT_MAX_LENGTH",Array("#CNT#"=>$arParams["MINUS_TEXT_MAX_LENGTH"]))."<br>";
		}
		...
		elseif($arParams["MIN_LENGTH"]>0 && strlen($MESSAGE_TEXT)<$arParams["MIN_LENGTH"])
			$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_MIN_LENGTH",Array("#CNT#"=>$arParams["MIN_LENGTH"]))."<br>";				
		elseif($arParams["MAX_LENGTH"]>0 && strlen($MESSAGE_TEXT)>$arParams["MAX_LENGTH"])
			$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_MAX_LENGTH",Array("#CNT#"=>$arParams["MAX_LENGTH"]))."<br>";

        $RATING = (int)$_POST["RATING"];
        if($arParams["REQUIRED_RATING"] && $RATING==0)
        {
            $arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_RATING")."<br>";
        }
...
//UF
        ...
        if(strlen($arResult["ERROR_MESSAGE"])==0)
        {
				if(!$USER->IsAuthorized())
				{
					$APPLICATION->set_cookie("REVIEW_AUTHOR_NAME", $arReview["AUTHOR_NAME"], false, "/", false, false, true, "ALTASIB_REVIEW");
					$APPLICATION->set_cookie("REVIEW_AUTHOR_EMAIL", $arReview["AUTHOR_EMAIL"], false, "/", false, false, true, "ALTASIB_REVIEW");
				}
                
                $Review = new aReview();
				$urlAddString = $_REQUEST["ALTASIB_AJAX_CALL"] == "Y" ? "" : "#review".$REVIEW_ID;
				if($arParams["IS_SEF"] === "Y")
...
$APPLICATION->AddHeadScript('/bitrix/components/altasib/review.add/templates/.default/js/cusel-min-2.5.js');
$APPLICATION->AddHeadScript('/bitrix/components/altasib/review.add/templates/.default/js/cusel_init.js');
$APPLICATION->AddHeadScript('/bitrix/components/altasib/review.add/templates/.default/js/jquery.jscrollpane.min.js');
$APPLICATION->AddHeadScript('/bitrix/components/altasib/review.add/templates/.default/js/jquery.mousewheel-3.0.4.js');
*/
?>
