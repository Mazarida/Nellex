...
else
{
    if ($arParams["CACHE_TIME"] > 0)
		...
		if($arElement = $rsElement->GetNext())
		{
            ...
            $arResult["ALL_CNT"] = Internals\ReviewTable::count(array_merge($arRFilter,array('APPROVED'=>'Y')));
            $limit = !$arParams["SHOW_ALL"] ? $arParams["REVIEWS_ON_PAGE"] : null;
            $obReview = Internals\ReviewTable::getList(array(
			...
			while($arReview = $obReview->fetch())
			{
                ...
                $arReview["FILES"] = Internals\FileTable::getFiles($arReview["ID"]);

                if($arParams["SHOW_AVATAR_TYPE"]!=false && $arReview["USER_ID"]>0)
                {
                    $arReview["USER_AVATAR"] = ALTASIB\Review\User::getAvatar($arReview["USER_ID"],$arParams["SHOW_AVATAR_TYPE"],Array("width"=>$arParams["AVATAR_WIDTH"],"height"=>$arParams["AVATAR_HEIGHT"]));
                }
...
var ReviewModMessages = {
    ...
    'REVIEW_LIST_MODER_SET_BEST': '".GetMessage("ALTASIB_REVIEW_T_REVIEW_LIST_MODER_SET_BEST")."',
    'REVIEW_LIST_MODER_DEL_BEST': '".GetMessage("ALTASIB_REVIEW_T_REVIEW_LIST_MODER_DEL_BEST")."'
	};
</script>");
