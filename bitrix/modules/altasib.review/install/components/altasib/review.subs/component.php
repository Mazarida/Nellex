...
if($_SERVER["REQUEST_METHOD"] == "POST" && check_bitrix_sessid() && $_POST["SUBS"]=="Y" && in_array($_POST["ACTION"],Array("SUBS","UNSUBS","CHECK")))
{
    ...
    if($_POST["ACTION"]==="CHECK")
    {
		...
		{
    		$arSubResult = Array("TYPE"=>false,"ERR"=>GetMessage("ALTASIB_CP_WRONG_CAPTCHA")."<br>");
    ...
    if($_POST["ACTION"]==="SUBS")
    {
        ...
        if($res)
        {
            $APPLICATION->set_cookie("REVIEW_AUTHOR_EMAIL", $Email, false, "/", false, false, true, "ALTASIB_REVIEW");
            $result = true;    
        }
        else
        {
    ...
    if($_POST["ACTION"]==="UNSUBS")
    {
        if($USER->IsAuthorized())
            ...
            $Email = $APPLICATION->get_cookie("REVIEW_AUTHOR_EMAIL", "ALTASIB_REVIEW");
