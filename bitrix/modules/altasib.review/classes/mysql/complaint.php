...
Class aReviewComplaint extends aReviewComplaintMain
{
        Function GetList($arOrder = Array("ID"=>"ASC"), $arFilter=Array())
        {
               ...
               $arSqlOrder = Array();
                ...
                $strSql = "
                SELECT DISTINCT RC.*
                FROM altasib_review_complaint RC
                WHERE
                "
                .$strSqlSearch
                .$strSqlOrder;
        ...
        Function Add($arFields)
        {
                ...
                if(intval($arFields["REVIEW_ID"])==0 || strlen($arFields["MESSAGE"])==0):
                    ...
                    $arInsert = $DB->PrepareInsert("altasib_review_complaint", $arFields, "altasib.review");

                    $strSql =
                        "INSERT INTO altasib_review_complaint (".$arInsert[0].") ".
                        "VALUES (".$arInsert[1].")";
        ...
        Function Update($ID, $arFields)
        {
                ...
                if((isset($arFields["COMMENT_ID"]) && intval($arFields["COMMENT_ID"])==0) || (isset($arFields["MESSAGE"]) && strlen($arFields["MESSAGE"])==0)):
                    ...
                    $arInsert = $DB->PrepareUpdate("altasib_review_complaint", $arFields, "altasib.review");

                    $strSql = "UPDATE altasib_review_complaint SET ".$arInsert." WHERE ID=".$ID;
        ...
        Function Delete($ID)
        {
                ...
                $strSql = "DELETE FROM altasib_review_complaint WHERE ID=".$ID;
                $res = $DB->Query($strSql, false, $err_mess.__LINE__);
        return true;
        }
}
