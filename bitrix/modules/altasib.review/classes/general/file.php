...
Class aReviewFile
{
        Function GetFiles($REVIEW_ID)
        {
                ...
                $arResult = Array();
                $strSql = "SELECT FILE_ID FROM altasib_review_file WHERE REVIEW_ID=".$REVIEW_ID;
                $res = $DB->Query($strSql);
        ...
        Function Delete($REVIEW_ID)
        {
                ...
                if($fe)
                {
                        $strSql = "DELETE FROM altasib_review_file WHERE REVIEW_ID=".$REVIEW_ID;
                        $DB->Query($strSql);
                }
        ...
        Function DeleteByFileID($FILE_ID)
        {
                ...
                $strSql = "SELECT FILE_ID,ELEMENT_ID FROM altasib_review_file WHERE FILE_ID=".$FILE_ID;
                $obCF = $DB->Query($strSql);
                if($arFile = $obCF->Fetch())
                {
                        CFile::Delete($arFile["FILE_ID"]);
                        $strSql = "DELETE FROM altasib_review_file WHERE FILE_ID=".$FILE_ID;
                        $DB->Query($strSql);
