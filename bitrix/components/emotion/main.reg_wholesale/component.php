	...
	if(check_bitrix_sessid())
	{
		if(empty($arParams["REQUIRED_FIELDS"]) || !in_array("NONE", $arParams["REQUIRED_FIELDS"]))
		{
			...
			if((empty($arParams["REQUIRED_FIELDS"]) || in_array("PATRONYMIC", $arParams["REQUIRED_FIELDS"])) && strlen($_POST["user_patronymic"]) <= 1)
				$arResult["ERROR_MESSAGE"][] = GetMessage("MF_REQ_PATRONYMIC");
			...
			if((empty($arParams["REQUIRED_FIELDS"]) || in_array("PHONE", $arParams["REQUIRED_FIELDS"])) && strlen($_POST["user_phone"]) <= 6)
		...
		if(empty($arResult["ERROR_MESSAGE"]))
		{
			$arFields = Array(
				...
				"AUTHOR_PATRONYMIC" => $_POST["user_patronymic"],
			...
			$_SESSION["MF_PATRONYMIC"] = htmlspecialcharsbx($_POST["user_patronymic"]);
		...
		$arResult["AUTHOR_PATRONYMIC"] = htmlspecialcharsbx($_POST["user_patronymic"]);
...
//	}
	...
	if(strlen($_SESSION["MF_PATRONYMIC"]) > 0)
		$arResult["AUTHOR_PATRONYMIC"] = htmlspecialcharsbx($_SESSION["MF_PATRONYMIC"]);
	if(strlen($_SESSION["MF_EMAIL"]) > 0)
		$arResult["AUTHOR_EMAIL"] = htmlspecialcharsbx($_SESSION["MF_EMAIL"]);
	if(strlen($_SESSION["MF_PHONE"]) > 0)
		$arResult["AUTHOR_PHONE"] = htmlspecialcharsbx($_SESSION["MF_PHONE"]);
	if(strlen($_SESSION["MF_CITY"]) > 0)
		$arResult["AUTHOR_CITY"] = htmlspecialcharsbx($_SESSION["MF_CITY"]);
}
