...
if ($arParams["ONLY_AUTH_SEND"] && !$USER->IsAuthorized()) {
	...
	echo GetMessage("ALTASIB_REVIEW_ADD_CP_ONLY_AUTH_SEND", array(
		"#REG_URL#" => $arParams['REG_URL'] . (strstr($arParams['REG_URL'], "?") ? "&" : "?") . "backurl=" . $back_url
	));
	return;
}
...
$arParams["ALLOW_EDIT"] = ($APPLICATION->GetGroupRight("altasib.review") >= "M") ? true : false;
...
$arUFBase = $GLOBALS["USER_FIELD_MANAGER"]->GetUserFields("ALTASIB_REVIEW", 0, LANGUAGE_ID);
...
if ($arParams["ELEMENT_ID"] == 0 && strlen($arParams["ELEMENT_CODE"]) == 0) {
	...
	if ($arParams["IS_SEF"] === "Y") {
		...
		$componentPage = $engine->guessComponentPath($arParams["SEF_BASE_URL"], array(
		...
		CComponentEngine::InitComponentVariables(false, $arComponentVariables, array(
			"SECTION_ID" => "SECTION_ID",
			"ELEMENT_ID" => "ELEMENT_ID",
			"ELEMENT_ID" => "ID",
			"SECTION_CODE" => "SECTION_CODE",
...
if ($arParams["ELEMENT_ID"] <= 0 && strlen($arParams["ELEMENT_CODE"]) > 0) {
	$arParams["ELEMENT_ID"] = CIBlockFindTools::GetElementID($arParams["ELEMENT_ID"], $arParams["ELEMENT_CODE"], false,
		false, array(
			"IBLOCK_ID" => $arParams["IBLOCK_ID"],
			"IBLOCK_LID" => SITE_ID,
			"IBLOCK_ACTIVE" => "Y",
...
if (!CIBlockElement::GetList(Array(),
	array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "ID" => $arParams["ELEMENT_ID"], "ACTIVE" => 'Y'), false, false,
	Array("ID", "IBLOCK_ID"))->Fetch()
) {
	return;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && check_bitrix_sessid() && (strlen($_POST["review_add_btn"]) > 0 || $_POST["ACTION"] == "EDIT")) {
	...
	if ($arParams["USE_CAPTCHA"] == "Y") {
		if (!$APPLICATION->CaptchaCheckCode($_POST["captcha_word"], $_POST["captcha_sid"])) {
			$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_REVIEW_ADD_CP_WRONG_CAPTCHA") . "<br>";
		}
	}
	$TITLE;
	if ($arParams["ALLOW_TITLE"]/* && !$arParams["COMMENTS_MODE"]*/) {
		...
		if ($arParams['TITLE_MIN_LENGTH'] > 0 && strlen($TITLE) < $arParams['TITLE_MIN_LENGTH']) {
			$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_TITLE_MIN_LENGTH",
					Array("#CNT#" => $arParams["TITLE_MIN_LENGTH"])) . "<br>";
		}

		if (strlen($TITLE) > 255) {
			$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_TITLE_MAX_LENGTH",
					Array("#CNT#" => $arParams["TITLE_MAX_LENGTH"])) . "<br>";
		}
	}
	$MESSAGE_PLUS_TEXT;
	$MESSAGE_MINUS_TEXT;
	if (!$arParams["COMMENTS_MODE"]) {
		...
		if (strlen($MESSAGE_PLUS_TEXT) == 0) {
			$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_MESSAGE_PLUS") . "<br>";
		} elseif ($arParams["PLUS_TEXT_MIN_LENGTH"] > 0 && strlen($MESSAGE_PLUS_TEXT) < $arParams["PLUS_TEXT_MIN_LENGTH"]) {
			$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_PLUS_TEXT_MIN_LENGTH",
					Array("#CNT#" => $arParams["PLUS_TEXT_MIN_LENGTH"])) . "<br>";
		} elseif ($arParams["PLUS_TEXT_MAX_LENGTH"] > 0 && strlen($MESSAGE_PLUS_TEXT) > $arParams["PLUS_TEXT_MAX_LENGTH"]) {
			$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_PLUS_TEXT_MAX_LENGTH",
					Array("#CNT#" => $arParams["PLUS_TEXT_MAX_LENGTH"])) . "<br>";
		}

		if ($arParams["MINUS_TEXT_MIN_LENGTH"] > 0 && strlen($MESSAGE_MINUS_TEXT) < $arParams["MINUS_TEXT_MIN_LENGTH"]) {
			$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_MINUS_TEXT_MIN_LENGTH",
					Array("#CNT#" => $arParams["MINUS_TEXT_MIN_LENGTH"])) . "<br>";
		} elseif ($arParams["MINUS_TEXT_MAX_LENGTH"] > 0 && strlen($MESSAGE_MINUS_TEXT) > $arParams["MINUS_TEXT_MAX_LENGTH"]) {
			$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_MINUS_TEXT_MAX_LENGTH",
					Array("#CNT#" => $arParams["MINUS_TEXT_MAX_LENGTH"])) . "<br>";
		}
	}
	$MESSAGE_TEXT = trim(htmlspecialcharsEx($_POST["MESSAGE"]));
	if (strlen($MESSAGE_TEXT) == 0) {
		$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_MESSAGE") . "<br>";
	} elseif ($arParams["MIN_LENGTH"] > 0 && strlen($MESSAGE_TEXT) < $arParams["MIN_LENGTH"]) {
		$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_MIN_LENGTH",
				Array("#CNT#" => $arParams["MIN_LENGTH"])) . "<br>";
	} elseif ($arParams["MAX_LENGTH"] > 0 && strlen($MESSAGE_TEXT) > $arParams["MAX_LENGTH"]) {
		$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_MAX_LENGTH",
				Array("#CNT#" => $arParams["MAX_LENGTH"])) . "<br>";
	}
	...
	if (!$USER->IsAuthorized()) {
		...
		if (strlen($arReview["AUTHOR_NAME"]) == 0) {
			$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_NAME") . "<br>";
		}

		$arResult["arMessage"]["AUTHOR_EMAIL"] = $arReview["AUTHOR_EMAIL"] = $_POST["AUTHOR_EMAIL"];
		if (!check_email($arReview["AUTHOR_EMAIL"])) {
			$arResult["ERROR_MESSAGE"] .= GetMessage("ALTASIB_CP_WRONG_EMAIL") . "<br>";
		}
	} else {
	...
	if (!$arParams["MODERATE"]) {
		$not_moderate = COption::GetOptionString("altasib.review", "not_moderate", "N");
		if ($not_moderate == "N") {
			$arReview["APPROVED"] = "Y";
		} else {
			if ($USER->GetID() > 0) {
				if ($arUser = Review\Internals\UserTable::getRow(['filter' => ['USER_ID' => $USER->GetID()]])) {
					if ($arUser["ALLOW_POST"] != "Y") {
						$arResult["ERROR_MESSAGE"] = GetMessage("ALTASIB_CP_WRONG_CLOSE");
					} else {
	...
	if ($arParams["MODERATE_LINK"]) {
		preg_match_all("#((http|https|ftp):\/\/[a-z:@,.'/\#\%=~\\&?*+\[\]_0-9\x01-\x08-]+)#ies", $MESSAGE_PLUS_TEXT,
			$arTextLinks);
		if (isset($arTextLinks[0]) && count($arTextLinks[0]) > 0) {
			$arReview["APPROVED"] = "N";
		}
		preg_match_all("#((http|https|ftp):\/\/[a-z:@,.'/\#\%=~\\&?*+\[\]_0-9\x01-\x08-]+)#ies", $MESSAGE_MINUS_TEXT,
			$arTextLinks);
		if (isset($arTextLinks[0]) && count($arTextLinks[0]) > 0) {
			$arReview["APPROVED"] = "N";
		}
		preg_match_all("#((http|https|ftp):\/\/[a-z:@,.'/\#\%=~\\&?*+\[\]_0-9\x01-\x08-]+)#ies", $MESSAGE_TEXT,
			$arTextLinks);
		if (isset($arTextLinks[0]) && count($arTextLinks[0]) > 0) {
			$arReview["APPROVED"] = "N";
		}
...
//UF
					...
					if (is_array($old_id)) {
						...
						foreach ($old_id as $key => $value) {
							$arUserFields[$arPostField["FIELD_NAME"]][$key] = array(
								...
								"del" => is_array($_POST[$arPostField["FIELD_NAME"] . "_del"]) && in_array($value,
										$_POST[$arPostField["FIELD_NAME"] . "_del"]),
								"old_id" => $value,
							);
						}
					} else {
	...
	if (strlen($arResult["ERROR_MESSAGE"]) == 0) {
		if (!$USER->IsAuthorized()) {
			$APPLICATION->set_cookie("REVIEW_AUTHOR_NAME", $arReview["AUTHOR_NAME"], false, "/", false, false, true,
				"ALTASIB_REVIEW");
			$APPLICATION->set_cookie("REVIEW_AUTHOR_EMAIL", $arReview["AUTHOR_EMAIL"], false, "/", false, false, true,
				"ALTASIB_REVIEW");
		}
		...
		if ($Edit) {
			$urlAddString = $_REQUEST["ALTASIB_AJAX_CALL"] == "Y" ? "" : "#review" . $EditID;
		}

		if ($arParams["IS_SEF"] === "Y") {
			$URL_VIEW = $APPLICATION->GetCurPage() . $urlAddString;
		} else {
			$URL_VIEW = $APPLICATION->GetCurPageParam("",
					array("login", "logout", "forgot_password", "change_password")) . $urlAddString;
		}
		...
		if ($Edit) {
			...
			if (!$Review->Update($EditID, $arReview)) {
				...
				if ($arParams["SAVE_RATING"]) {
					aReview::SaveRatingToIB($arParams["ELEMENT_ID"], $arParams["IBLOCK_ID"],
						$arParams["SAVE_RATING_IB_PROPERTY"]);
				}
			...
			if (!$REVIEW_ID = $Review->Add($arReview, false)) {
				...
				$URL_VIEW .= $_REQUEST["ALTASIB_AJAX_CALL"] == "Y" ? "" : "#review" . $REVIEW_ID;
				//rating save
				...
				if ($arParams["SAVE_COUNT"] && $arReview["APPROVED"] == 'Y') {
					Review\Tools::saveCountToProperty($arParams["ELEMENT_ID"], $arParams["IBLOCK_ID"],
						$arParams["SAVE_COUNT_IB_PROPERTY"]);
				}
				...
				if (strlen($MESSAGE_PLUS_TEXT) > 0) {
					$arEventFields["MESSAGE_BODY"] .= GetMessage('ALTASIB_CP_EV_PLUS') . CEvent::GetMailEOL() . CTextParser::convert4mail($MESSAGE_PLUS_TEXT) . CEvent::GetMailEOL();
				}

				if (strlen($MESSAGE_MINUS_TEXT) > 0) {
					$arEventFields["MESSAGE_BODY"] .= GetMessage('ALTASIB_CP_EV_MINUS') . CEvent::GetMailEOL() . CTextParser::convert4mail($MESSAGE_MINUS_TEXT) . CEvent::GetMailEOL();
				}

				if (strlen($MESSAGE_TEXT) > 0) {
					$arEventFields["MESSAGE_BODY"] .= GetMessage('ALTASIB_CP_EV_COMMENT') . CEvent::GetMailEOL() . CTextParser::convert4mail($MESSAGE_TEXT);
				}
				...
				$arEventFields["EMAIL_FROM"] = COption::GetOptionString("altasib.review", "email_from");
				$arEventFields["EMAIL_ADMIN"] = COption::GetOptionString("altasib.review", "email_admin");

				$db_events = GetModuleEvents("altasib.review", "OnBeforeAddSendMail");
				...
				foreach ($arSendEmails as $Email) {
					...
					CEvent::SendImmediate("ALTASIB_REVIEW_ADD", SITE_ID, $arEventFields);
				}

				if ($arParams['SEND_NOTIFY']) {
					$arEventFields["EMAIL"] = $arEventFields["AUTHOR_EMAIL"];
					CEvent::SendImmediate("ALTASIB_REVIEW_ADD_NOTIFY", SITE_ID, $arEventFields);
				}
...
if ($_POST["RID"] > 0 && $_POST["ACTION"] == "EDIT" && $arParams["ALLOW_EDIT"] && empty($arResult["ERROR_MESSAGE"])) {
	...
	$arUFBase = $GLOBALS["USER_FIELD_MANAGER"]->GetUserFields("ALTASIB_REVIEW", $REVIEW_ID_EDIT, LANGUAGE_ID);
	$arParams["USER_FIELDS"] = array();
	$arParams["USER_FIELDS_RATING"] = Array();
	foreach ($arUFBase as $k => $arUF) {
		if ($arUF['USER_TYPE_ID'] == 'ALTASIB_REVIEW_RATING') {
...
<script type="text/javascript">
	...
	BX.ready(function () {
			BX.message({
				ERROR_FILE_UPLOAD: '<?=GetMessageJS('ALTASIB_REVIEW_ADD_ERROR_UPLOAD_FILE')?>'
			})
		}
	);
</script>