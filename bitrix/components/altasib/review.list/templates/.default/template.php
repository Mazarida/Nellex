...
<div class="alx_reviews_list" id="alx_reviews_list">
	<div class="alx_reviews_title">
		<div class="alx_reviews_title_txt"><? echo strlen($arParams["LIST_TITLE"]) > 0 ? $arParams["LIST_TITLE"] : GetMessage("ALTASIB_REVIEW_T_REVIEW_LIST_REVIEW") ?>:</div>
		<div class="alx_reviews_list_count">
			<div class="alx_reviews_count_bg"></div>
			<span id="review_cnt"><?= $arResult["ALL_CNT"] ?></span></div>
	</div>
	<? foreach ($arResult["ITEMS"] as $arReview):
		$arReview['ALLOW_VOTE'] = false;
		if (Review::allowVote($arReview["ID"]) || ($USER->GetID() > 0 && $USER->GetID() == $arReview["USER_ID"])) {
			$arReview['ALLOW_VOTE'] = true;
		}
		?>
		<? if ($arReview["APPROVED"] == "N" && (($USER->GetID() == $arReview["USER_ID"] && $USER->GetID() > 0) || ($APPLICATION->get_cookie("REVIEW_AUTHOR_EMAIL",
					"ALTASIB_REVIEW") == $arReview["AUTHOR_EMAIL"] && $APPLICATION->get_cookie("REVIEW_AUTHOR_NAME",
					"ALTASIB_REVIEW") == $arReview["AUTHOR_NAME"]))
	) {
		...
		<div class="alx_reviews_item<? if ($arReview["APPROVED"] != "Y"): ?> hide<? endif; ?><? if ($arReview["IS_BEST"] == "Y"): ?> best<? endif; ?>" id="review-list-review-<?= $arReview["ID"] ?>">
			...
			<? if ($arParams["ALLOW_VOTE"]): ?>
				<div class="alx_reviews_item_vote_result alx_reviews_item_vote_result_<?= $class ?>">
					<div class="alx_reviews_item_vote_result_arr"></div>
					<span id="review_vote_r<?= $arReview["ID"] ?>"><?= $arReview["VOTE"]; ?></span></div>
			<? endif; ?>
			<div class="alx_reviews_item_author_info">
				<div class="alx_reviews_ava"><? if ($arParams["SHOW_AVATAR_TYPE"] != false && strlen($arReview["USER_AVATAR"]) > 0): ?>
						<img src="<?= $arReview["USER_AVATAR"] ?>" alt="<?= $arReview["AUTHOR_NAME"]; ?>" border="0" /><? else: ?>
						<img src="<?= $templateFolder; ?>/images/no_photo.png" alt="" border="0" /><? endif; ?></div>
				<div class="alx_reviews_user_name"><? if (strlen($arReview["SHOW_USER_PATH"]) > 0): ?>
					<a href="<?= $arReview["SHOW_USER_PATH"] ?>" target="_blank" rel="nofollow"><? endif;
						?><?= $arReview["AUTHOR_NAME"]; ?><? if (strlen($arReview["SHOW_USER_PATH"]) > 0): ?></a><? endif;
				?></div>
			...
			<div class="alx_reviews_vote_item">
				...
				<? if (!empty($arReview["USER_FIELDS_RATING"])): ?>
					<? if ($arParams["SHOW_MAIN_RATING"]): ?>
						<div class="alx_reviews_item_vote_show">
							<a href="javascript:void(0)" onclick="jsReview.ShowVotes(<?= $arReview["ID"] ?>)"><?= GetMessage('ALTASIB_REVIEW_T_REVIEW_LIST_SHOW_ALL_VOTES') ?></a>
						</div>
					<? endif; ?>
			...
			<div class="alx_reviews_item_sec_list" id="review_item_e_<?= $arReview["ID"] ?>">
				<? if (!$arParams['COMMENTS_MODE']): ?>
					<div class="alx_reviews_item_sec">
						<div class="alx_reviews_title_caps"><?= GetMessage("ALTASIB_REVIEW_T_REVIEW_LIST_MESSAGE_PLUS") ?>:</div>
						<div class="alx_review_mess"><?= $arReview["MESSAGE_PLUS_HTML"]; ?></div>
					</div>
					<div class="alx_reviews_item_sec">
						<div class="alx_reviews_title_caps"><?= GetMessage("ALTASIB_REVIEW_T_REVIEW_LIST_MESSAGE_MINUS") ?>:</div>
						<div class="alx_review_mess"><?= $arReview["MESSAGE_MINUS_HTML"]; ?></div>
					</div>
				<? endif; ?>
				<div class="alx_reviews_item_sec">
					<div class="alx_reviews_title_caps"><?= GetMessage("ALTASIB_REVIEW_T_REVIEW_LIST_MESSAGE") ?>:</div>
						...
						if ((is_array($arUserField["VALUE"]) && count($arUserField["VALUE"]) > 0) || (!is_array($arUserField["VALUE"]) && StrLen($arUserField["VALUE"]) > 0)):
							...
							<div class="alx_reviews_item_sec">
								...
								<div class="alx_review_mess">
									<? $APPLICATION->IncludeComponent("bitrix:system.field.view",
										$arUserField["USER_TYPE"]["USER_TYPE_ID"], array("arUserField" => $arUserField),
			...
			<div class="alx_reviews_item_link">
				<noindex>
					<a rel="nofollow" href="#review<?= $arReview["ID"]; ?>"><?= GetMessage('ALTASIB_REVIEW_T_REVIEW_LINK') ?></a>
				</noindex>
			</div>
			<? if ($arParams["ALLOW_VOTE"]): ?>
				<div class="alx_reviews_item_vote_do">
					<div class="alx_rev_vot_titl"><?= GetMessage("ALTASIB_REVIEW_T_REVIEW_LIST_VOTE") ?></div>
	...
	<? if ($arResult["ALL_CNT"] > $arParams["REVIEWS_ON_PAGE"] && !$arParams["SHOW_ALL"]): ?>
		<a href="javascript:void(0)" onclick="BX.ajax.get(CURRENT_URL,{'ALTASIB_AJAX_CALL' : 'Y','showAll' : 'Y'},function (res) {BX.showWait(BX('#alx_reviews_list'));BX('alx_reviews_list').innerHTML = res;BX.closeWait(BX('#alx_reviews_list'));})" class="alx_reviews_show_more">
			<?= GetMessage("ALTASIB_REVIEW_T_REVIEW_LIST_SHOW_ALL") ?>
			<span class="alx_reviews_count_all"><?= $arResult["ALL_CNT"] - $arParams["REVIEWS_ON_PAGE"] ?></span> </a>
	<? endif; ?>
</div>