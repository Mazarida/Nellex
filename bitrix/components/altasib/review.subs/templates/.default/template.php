...
var SubBut = '<?=GetMessage("ALTASIB_TP_SUBS_BUT")?>';
</script>
...
<input type="hidden" value="UNSUBS" name="ACTION">
    (<a href="javascript:void(0)" onclick="document.forms.review_subs.submit();"><?=GetMessage("ALTASIB_TP_UNSUBS")?></a>)
<?else:?>
<input type="hidden" value="SUBS" name="ACTION">
    (<a href="javascript:void(0)" <?if(!$USER->IsAuthorized()):?>onclick="ElSub();return false;"<?else:?>onclick="document.forms.review_subs.submit();"<?endif;?>><?=GetMessage("ALTASIB_TP_SUBS")?></a>)
<?endif;?>
</form>
</div>