...
if (!in_array($arParams["NAME_SHOW_TYPE"],
	Array("LOGIN", "NAME", "LOGIN_NAME", "NAME_LAST_NAME", "LOGIN_NAME_LAST_NAME"))) {
...
if ($this->StartResultCache(false, array($arNavigation))) {
	...
	$data = ALTASIB\Review\Internals\ReviewTable::getList(array(
		'select' => array(
			'ID',
			'ELEMENT_ID',
			'USER_ID',
	...
	while ($Review = $result->fetch()) {
		...
		if (strlen($arParams['USER_PATH']) > 0 && $Review["USER_ID"]) {
			$Review["SHOW_USER_PATH"] = CComponentEngine::MakePathFromTemplate($arParams['USER_PATH'],
				Array("USER_ID" => $Review["USER_ID"]));
		}

		if ($Review["USER_ID"] > 0) {
			switch ($arParams["NAME_SHOW_TYPE"]) {
				case "LOGIN":
					$Review["AUTHOR_NAME"] = $Review["ALTASIB_REVIEW_REVIEW_USER_LOGIN"];
					break;

				case "NAME":
					$Review["AUTHOR_NAME"] = $Review["ALTASIB_REVIEW_REVIEW_USER_NAME"];
					break;
				case "NAME_LAST_NAME":
					$Review["AUTHOR_NAME"] = $Review["ALTASIB_REVIEW_REVIEW_USER_NAME"] . " " . $Review["ALTASIB_REVIEW_REVIEW_USER_LAST_NAME"];
					break;
				case "LOGIN_NAME":
					$Review["AUTHOR_NAME"] = "(" . $Review["ALTASIB_REVIEW_REVIEW_USER_LOGIN"] . ") " . $Review["ALTASIB_REVIEW_REVIEW_USER_NAME"];
					break;
				case "LOGIN_NAME_LAST_NAME":
					$Review["AUTHOR_NAME"] = "(" . $Review["ALTASIB_REVIEW_REVIEW_USER_LOGIN"] . ") " . $Review["ALTASIB_REVIEW_REVIEW_USER_NAME"] . " " . $Review["ALTASIB_REVIEW_REVIEW_USER_LAST_NAME"];
					break;
			}
		}
		...
		if ($arParams["SHOW_AVATAR_TYPE"] != false && $Review["USER_ID"] > 0) {
			$Review["USER_AVATAR"] = ALTASIB\Review\User::getAvatar($Review["USER_ID"], $arParams["SHOW_AVATAR_TYPE"],
				Array("width" => $arParams["AVATAR_WIDTH"], "height" => $arParams["AVATAR_HEIGHT"]));
		}

		$Review["USER_FIELDS"] = $GLOBALS["USER_FIELD_MANAGER"]->GetUserFields("ALTASIB_REVIEW", $Review["ID"],
			LANGUAGE_ID);
		$Review["USER_FIELDS_RATING"] = Array();
		foreach ($Review["USER_FIELDS"] as $k => $arUF) {

			if ($arUF['USER_TYPE_ID'] == 'ALTASIB_REVIEW_RATING') {
				if (empty($arParams["UF_VOTE"]) || (count($arParams["UF_VOTE"]) > 0 && !in_array($k,
							$arParams["UF_VOTE"]))) {
					continue;
				}
	...
	$countQuery = new Bitrix\Main\Entity\Query(ALTASIB\Review\Internals\ReviewTable::getEntity());
	$countQuery->registerRuntimeField("CNT", array(
		...
		->setSelect(array("CNT"))
		->setFilter($filter);
	$totalCount = $countQuery->setLimit(null)
		->setOffset(null)
		->exec()
		->fetch();
